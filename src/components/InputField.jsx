...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      w-full p-4 rounded-xl bg-[#030014]/10 text-white placeholder-transparent 
      focus:outline-none focus:ring-2 focus:ring-[#4FACFE] focus:ring-offset-2 
      focus:ring-offset-[#1c1e26] transition-all duration-300 peer
    `;

    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(99,102,241,0.4)] border-[#4FACFE]"
      : "border-white/20 hover:border-[#4FACFE]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#4FACFE]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#4FACFE] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#4FACFE]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
